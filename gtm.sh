...
DNS_IPS=(
 "p0.freedns.controld.com.masa.my.id"
 "124.6.181.12"
 "124.6.181.4"
)
NAME_SERVERS=(
 "dns.sg82gg.devsiple.com"
)
TARGET_ADDRESS="76.76.2.0"
...
check(){
  ...
  if [ -n "$ping_result" ]; then
    ...
    if (( ping_time <= 500 )); then  # Adjust the condition as needed
      PING_STATUS="Ping Time: \e[92m${ping_time} ms\e[0m"  # Green color for ping time below or equal to 500ms
    else
      PING_STATUS="Ping Time: \e[91m${ping_time} ms\e[0m"  # Red color for ping time greater than 500ms
    fi
  else
  ...
  for T in "${DNS_IPS[@]}"; do
    for R in "${NAME_SERVERS[@]}"; do
      result=$(${_DIG} @${T} ${R} +short 2>/dev/null || echo "Failed")
      if [ -z "$result" ]; then
